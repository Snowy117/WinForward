...
- The implicit forwarded pass is cached in `FlowTable` (`src/WinForward.Core/Domain.cs`); reverse and cross-adapter observations reuse it before origin-specific policy can run again. Flow-table capacity exhaustion still fails closed.

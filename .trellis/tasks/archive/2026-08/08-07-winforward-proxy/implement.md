...
### Review-fix pass (2026-08-07, trellis-check findings resolved)

A check-phase review (trellis-check) found one CRITICAL and three WARNINGs plus INFOs; all are fixed and verified. 0 warnings / 0 errors, 129/129 unit tests.

- **CRITICAL (fixed): TCP upstream loop-prevention registration never matched.** `TcpProxyRelayFactory` registered `(Tcp, proxy:socks, proxy:socks)`, which can never match the observed upstream SYN `(host:ephemeral -> proxy:socks)`; under a catch-all proxy rule the SOCKS5 control connection would be recursively re-proxied. Fix: `Socks5ControlConnection.ConnectAsync` now binds the socket to a wildcard local endpoint, invokes an `onSocketReady` callback BEFORE the SYN leaves the host, and owns the returned registration token (disposed with the connection). Both `TcpProxyRelayFactory` and `Socks5UdpTransport.CreateAsync` register the control connection's exact `(Any:port -> proxy:socks)` tuple; the UDP path previously registered only the relay socket, leaving its TCP control connection unprotected too. Locked by `SelfTrafficUpstreamTcpTupleIsOwnedWhenObservedAsHostEphemeralToProxy` (forward + reverse owned; an unrelated app on the same proxy endpoint is not exempted).
- **WARNING (fixed): adapterId+adapterName same-adapter rule.** `CaptureAdapterScopeResolver` previously resolved each field independently with no cross-check, so `adapterId: [A]` + `adapterName: [B]` (A≠B) passed startup and could never match. `ResolveRuleScope` now requires both fields to resolve to the same adapter set (AND semantics) and fails startup with a diagnostic. Locked by `AdapterScopeFailsWhenAdapterIdAndAdapterNameResolveToDifferentAdapters` / `AdapterScopeAcceptsAdapterIdAndAdapterNameResolvingToTheSameAdapter`.
- **WARNING (fixed): idle expiry was never wired.** `FlowTable.RemoveExpired`, `TcpRedirectTable.RemoveExpired`, and `UdpAssociationTable.RemoveExpired` existed with no caller; UDP sessions accumulated to the bounded capacity then failed closed. Added `IdleExpirySweeper` (Runtime) started in `Program.RunCaptureLoopAsync` that periodically sweeps `FlowDispatcher.RemoveExpiredFlows`, `TcpProxyCoordinator.RemoveExpiredAsync`, and `UdpProxyCoordinator.RemoveExpiredAsync`. `UdpProxySession` now tracks last-activity (send + receive) and the coordinator claims/releases associations through `UdpAssociationTable` (relay-alias collision guard is now live, design §8). Locked by `RemoveExpiredDisposesIdleSessionAndReleasesAssociation` / `RemoveExpiredKeepsActiveSession`.
- **WARNING (fixed): README DLL-loading overclaim.** The README claimed the DLL is loaded from the application directory only, but no `DllImportResolver` existed. `NdisApiNative` now registers `NativeLibrary.SetDllImportResolver` that loads `ndisapi.dll` only from `AppContext.BaseDirectory`.
- **INFO (fixed): SOCKS5 REP status never surfaced.** `Socks5State.TryParseReply` required `reply[1] == 0`, so failure statuses were generic. It now returns the REP status for failure replies and `Socks5Messages.DescribeReplyStatus` maps RFC 1928 codes; `ReadEndpointReplyAsync` reports e.g. `SOCKS5 command failed: connection refused (REP 5).`
- **INFO (fixed): `TryResolveByProxyPort` O(n) per-packet scan.** `TcpRedirectTable` now keeps an O(1) `_byProxyPort` index maintained on claim/remove/expire.
- **INFO (fixed): misleading executor comment.** `NdisPacketActionExecutor.ProxyAsync` said NotRelevant mid-flow data is "passed through by normal policy handling"; it is in fact dropped (lease already ProxyConsumed). Comment corrected.
- **INFO (fixed): open-path imports lacked `SetLastError`.** `OpenFilterDriver`/`GetDriverVersion`/`GetTcpipBoundAdaptersInfo` now use `SetLastError = true`; `NdisApiDriver.Open`/`GetAdapters` report native error codes.
...
- [x] **9 (UDP relay hardware-verified on Win11):** UDP DNS queries via SOCKS5 UDP ASSOCIATE verified end-to-end (`UDP ASSOCIATE requested` -> `UDP relay bound`) — `nslookup example.com 192.168.77.1` through a `proxy` rule for nslookup.exe/UDP returns DNS answers via the relay, 3/3, with dynamic relay port != SOCKS TCP port. Hardware bring-up fixes: (1) **UDP relay self-loop** — the relay socket binds 0.0.0.0 but emits source-IP-chosen-by-routing, so self-traffic registry now wildcard-matches a registered Any/IPv6Any local endpoint by port+remote (`SelfTrafficRegistry.MatchesWildcard`, locked by test); (2) **UDP response redirection** — a UDP proxy flow's reverse datagram (server->client, the reinjected relay response) must be delivered to the local client, not re-proxied back to the relay (FlowDispatcher detects reverse-of-stored-key on a proxied UDP flow and passes it — locked by the updated FlowDispatcherTests; a missing pass here loops forever). Known test-harness artifact: the local python SOCKS5 server's own forwarded queries get re-caught and re-proxied (self-traffic protects WinForward's own sockets, not the SOCKS5 server's) — a remote SOCKS5 server avoids this; the nslookup path (short single query, answered before re-interception matters) is the clean hardware proof. 123/123 suite green. Remaining: Hyper-V forwarded UDP on a real VM host, IPv6 UDP, full matrix.

...
public class TcpResetBuilderTests
{
    ...
    public void TryBuildResetRejectsTooSmallDestinationWithoutWriting()
    {
        ...
        Assert.False(TcpResetBuilder.TryBuildReset(template, IPAddressValue.From(s_serverV4), 443, IPAddressValue.From(s_clientV4), 53000, 1001, 2002, destination, out var written));
    ...
    public void BuildResetFromSynRejectsUnparseableFrames()
    {
        ...
        var arp = BuildTemplateSyn(0x0806, 0x11, 0x22);
        Assert.Null(BuildResetFromSyn(arp, IPAddressValue.From(s_serverV4), 443, IPAddressValue.From(s_clientV4), 53000));
    }
    ...
    private static byte[]? BuildReset(ReadOnlySpan<byte> synFrame, IPAddress server, ushort serverPort, IPAddress client, ushort clientPort, uint serverSequenceNext, uint clientSequenceNext)
    {
        ...
        return TcpResetBuilder.TryBuildReset(synFrame, IPAddressValue.From(server), serverPort, IPAddressValue.From(client), clientPort, serverSequenceNext, clientSequenceNext, frame, out var written)
            ? frame[..written].ToArray()
            : null;
    }

    private static byte[]? BuildResetFromSyn(ReadOnlySpan<byte> synFrame, IPAddressValue server, ushort serverPort, IPAddressValue client, ushort clientPort)
    {
        Span<byte> frame = stackalloc byte[TcpResetBuilder.MaxResetFrameLength];

...
public sealed class UdpProxyCoordinatorLifecycleTests
{
    ...
    public async Task FailedSetupReleasesSlotAndCapacityForOtherFlows()
    {
        ...
        Assert.True(await coordinator.TrySendSpanAsync(first, s_server, new byte[] { 1 }, default, CancellationToken.None));
        ...
        Assert.True(await WaitUntilTrueAsync(() => coordinator.TrySendSpanAsync(second, s_server, new byte[] { 2 }, default, CancellationToken.None).AsTask()));
    ...
    public async Task ReceiveFaultDisposesAndRemovesSessionWithoutAnotherSend()
    {
        ...
        Assert.True(await WaitUntilTrueAsync(() => coordinator.TrySendSpanAsync(CreateFlow("192.0.2.54"), s_server, new byte[] { 2 }, default, CancellationToken.None).AsTask()));
        await WaitForAsync(() => factory.Transports.Count == 2);
        Assert.Equal(2, factory.Transports.Count);
    }
    ...
    public async Task ConcurrentDisposalIsSingleFlightAndRejectsNewSends()
    {
        ...
        await Assert.ThrowsAsync<ObjectDisposedException>(async () =>
            await coordinator.TrySendSpanAsync(CreateFlow("192.0.2.53"), s_server, new byte[] { 1 }, default, CancellationToken.None));
    ...
    public async Task ActiveSessionRetainsRelayAliasAfterItsCreationTimestampExpires()
    {
        ...
        time.Advance(TimeSpan.FromMinutes(2));
        Assert.True(await coordinator.TrySendSpanAsync(first, s_server, new byte[] { 2 }, default, CancellationToken.None));
        Assert.Equal(0, await coordinator.RemoveExpiredAsync(time.GetUtcNow(), TimeSpan.FromMinutes(1)));
        ...
        Assert.True(await coordinator.TrySendSpanAsync(second, s_server, new byte[] { 3 }, default, CancellationToken.None));
        Assert.True(await WaitUntilTrueAsync(async () => !await coordinator.TrySendSpanAsync(second, s_server, new byte[] { 4 }, default, CancellationToken.None)));
        time.Advance(TimeSpan.FromSeconds(1));
        Assert.True(await coordinator.TrySendSpanAsync(second, s_server, new byte[] { 5 }, default, CancellationToken.None));
    }
